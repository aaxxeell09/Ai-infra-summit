...
The current screen calls `createConfirmedComparisonRequest(snapshot, confirmed, mode, promptId, requestId)`. It returns `local-turbo.comparison-request.v1`, a shared prompt, sequential execution, the measured automatic placement (NPU HTP0) and, in speed mode, the CPU/10 fast profile from the same weights. The legacy `createComparisonRequest(snapshot, selectedRow, metric, mode, promptId, requestId)` remains for screening adapters and tests. In routing mode the selected model is null and routing explicitly has `pending_calibration` status. Candidate role names are editorial examples, not real model IDs or classifier decisions.
...
- **Speed:** same weights/quantization and equivalent workload controls. Never report shorter output as faster native decoding. The current comparison is automatic NPU against measured CPU/10; preserve those effective configurations in a live run.
