...
- The commands above do not opt into energy measurement. The subsequent [task-energy protocol](energy-protocol.md) adds `--energy-protocol` to the same runner and embeds per-task observations in its JSON results. Historical full-process telemetry sidecars remain separate evidence; they are not substituted for the new warm-task intervals.
